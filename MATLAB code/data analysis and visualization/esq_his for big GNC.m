...
savehis = 'BDAC_N800_R92_r14_ps1_Air_785_E2_his_';
...
%for smooth nanoshells R92_R169
...
% plotting area are r between 90~120, 120~150, 150~180 
xp(dist<dist_min)=[];% inside of PS core, there's no field
yp(dist<dist_min)=[];
zp(dist<dist_min)=[];
e2linlogsel(dist_min)=[];%inside of PS core, the E2=0
xp(dist>dist_max)=[];% inside of PS core, there's no field
yp(dist>dist_max)=[];
zp(dist>dist_max)=[];
e2linlogsel(dist>distb)=[];%inside of PS core, the E2=0
...
%%min = cutoff;
%%max = 5.2;% for 94.5 nm core, need to modify for 184 nm core
%%number_of_ticks = 5;
...
%%axis image
%%saveas(hcb2, 'colorbar_core94p5', 'tif');
%%saveas(hcb2, 'colorbar_core94p5', 'fig');
